...
* **roles and permissions** in `base3system_sysrole`, `base3system_syspermission`, `base3system_sysrolepermission`, `base3system_sysuserrole`, and `base3system_sysgrouprole`
...
Memora entry access is intentionally entry-specific and is based on direct user and group grants.

The active entry ACL tables are:

* `base3system_sysuseraccess`
* `base3system_sysgroupaccess`

Roles and permissions are not stored as entry-level ACL rows. They describe general capabilities such as administration, creation, and management rights. The old `base3system_sysroleaccess` model is retired and must not be used by active Memora code.
...
* `visitor` gives read access
* `moderator` gives edit access
* `owner` gives edit access and records entry-specific ownership

`owner` is deliberately user-entry specific. It is not a global role and should not be modeled as a permission.
...
Typical public/default rows are still represented through `sysuseraccess` or `sysgroupaccess`, not through roles.

### Effective Access

When `loadaccess` is used, Memora resolves the current user context into an effective access string:

* `edit`
* `view`
* `none`

The effective result is derived from:

```text
1. direct user access rows
2. group access rows inherited through user group membership
3. public/default access rows where applicable
4. admin bypass through the Usermanager permission model
```

Typical mapping:

```text
Permission entry/admin  -> edit
...
Admins are no longer represented through `sysroleaccess`.

The admin bypass is checked through the active Usermanager permission model:
...
If this returns `true`, query filtering and `loadaccess` may treat the current user as having `edit` access to entries.

This requires the active Usermanager implementation to return effective roles and permissions correctly. A stale external Usermanager that only returns `User::$role` without `roles` and `permissions` is not sufficient for the new RBAC path.

### Query Protection

Memora applies access restrictions at query level through access-related query extensions, so users only see entries they are allowed to access.

The conceptual condition is:

```text
entry is visible when
  current user has Permission::for('entry', 'admin')
  OR direct user access matches
  OR group access matches through current user group membership
  OR public/default user or group access applies
```

A direct `getEntry($id)` lookup should not be hidden by user profile filters, but it still remains protected by ACL/admin checks.

Roles and permissions are described in detail in [Roles and Permissions](#roles-and-permissions).
...
There is deliberately no compatibility alias for the old `MemoraApiIMemoraProfileService`.
...
MemoraAccessService    -> replaceuseraccess, replacegroupaccess
```
...
MemoraAccessService     -> roles, permissions, role permissions, user roles, group roles, user groups
...
Memora supports RBAC administration through ResourceFoundation, but RBAC is separate from entry-level ACL.

Entry-level access remains stored only in:
...
Reusable roles and permissions are stored in:
...
The old table below is retired:

```text
base3system_sysroleaccess
```

It should not exist in active schemas, should not be registered by the query schema provider, and should not be used by create/update payloads.

### Separation of Responsibilities

The current model deliberately separates two questions:

```text
Who may access this concrete entry?
What general capabilities does this user have?
```

Entry ACL answers the first question:

```text
entry <-> user  with mode visitor | moderator | owner
entry <-> group with mode visitor | moderator
```

RBAC answers the second question:
...
This keeps ownership and per-entry collaboration separate from reusable authorization capabilities.
...
Roles are named responsibility bundles.
...
A role should not contain `scope` or `permission` columns. Permission semantics are attached through `base3system_sysrolepermission`.
...
A permission is atomic and is addressed by the pair:
...
In PHP, permissions are usually addressed through the Usermanager model:
...
This table connects reusable roles to reusable permissions.
...
role admin -> entry/create
role admin -> user/manage
role admin -> group/manage
role admin -> role/manage
```

### User and Group Role Assignment

Roles can be assigned directly to users:
...
Effective user roles are the union of:

```text
1. roles directly assigned to the user
2. roles assigned to groups the user belongs to
```

Effective permissions are the permissions attached to those effective roles.

### Admin and Entry Permissions

The key permission for bypassing entry ACL is:
...
If the active Usermanager reports that the current user can `entry/admin`, Memora treats the user as an entry admin.
...
if ($usermanager->can(Permission::for('entry', 'admin'))) {
        // Current user may bypass normal entry ACL filtering.
}
```

`entry/admin` is not the same as being owner of each entry. It is an administrative bypass for entry access checks.

Entry-specific ownership still belongs to:
...
It manages roles and permissions without exposing Memora's internal table handling to consumers.
...
$roleId = $accessService->createRole([
        'name' => 'content_editor',
        'label' => 'Content Editor',
        'info' => 'May create and edit content entries'
]);
```
...
```php
$permissionId = $accessService->createPermission([
        'scope' => 'entry',
        'permission' => 'create',
        'label' => 'Create entries',
        'info' => 'May create entries'
]);
```
...
```php
$accessService->replaceRolePermissions($roleId, [
        $permissionId
]);
```

A role can also be created with permission references when the implementation supports the payload shape:
...
Replace the direct roles of one user:

```php
$accessService->replaceUserRoles(2, [
        $roleId
]);
```

Replace the roles of one group:

```php
$accessService->replaceGroupRoles(1, [
        $roleId
]);
```

Read effective roles for a user:

```php
$roles = $accessService->getEffectiveUserRoles(2);
```

Read permissions for a role:

```php
$permissions = $accessService->getRolePermissions($roleId);
```

### Entry Access Is Still User/Group Only

Create payloads may include entry ACL rows like this:
...
Update payloads may replace entry ACL rows like this:

```php
$memora->updateEntry($id, [
        'replaceuseraccess' => [
                [
                        'user_id' => 2,
                        'mode' => 'owner'
...
The following payload keys are retired and must not be used:
...
The following service method is also retired and must not be exposed in active ResourceFoundation implementations:

```text
replaceEntryRoleAccess()
```

### Migration Notes

A current Memora installation should use:
...
When migrating from the old role-access model:

1. keep `base3system_sysuseraccess` and `base3system_sysgroupaccess`
2. remove `base3system_sysroleaccess` from schema registration
3. remove create/update role-access extensions
4. create `base3system_syspermission`
5. create `base3system_sysrolepermission`
6. move role semantics from `sysrole.scope/sysrole.permission` into permissions
7. assign permissions to roles through `sysrolepermission`
8. make the active Usermanager return effective roles and permissions
